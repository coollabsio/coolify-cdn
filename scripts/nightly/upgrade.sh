...
nohup bash -c "
    ...
    fail_compose_up() {
        ...
        if [ -n \"\$context\" ]; then
            log \"ERROR: Docker compose up failed (\$context) with exit code \$exit_code\"
        else
            log \"ERROR: Docker compose up failed with exit code \$exit_code\"
        fi

        write_status 'error' 'Docker compose up failed'
        exit \"\$exit_code\"
    }
    ...
    if [ -f /data/coolify/source/docker-compose.custom.yml ]; then
        ...
        if docker run -v /data/coolify/source:/data/coolify/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/coollabsio/coolify-helper:\${LATEST_HELPER_VERSION} bash -c \"LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/coolify/source/.env -f /data/coolify/source/docker-compose.yml -f /data/coolify/source/docker-compose.prod.yml -f /data/coolify/source/docker-compose.custom.yml up -d --remove-orphans --wait --wait-timeout 60\" >>\"\$LOGFILE\" 2>&1; then
            log 'Docker compose up completed'
        else
            fail_compose_up \"\$?\" 'custom configuration'
        fi
    else
        log 'Using standard docker-compose configuration'
        log 'Running docker compose up...'
        if docker run -v /data/coolify/source:/data/coolify/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/coollabsio/coolify-helper:\${LATEST_HELPER_VERSION} bash -c \"LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/coolify/source/.env -f /data/coolify/source/docker-compose.yml -f /data/coolify/source/docker-compose.prod.yml up -d --remove-orphans --wait --wait-timeout 60\" >>\"\$LOGFILE\" 2>&1; then
            log 'Docker compose up completed'
        else
            fail_compose_up \"\$?\" ''
        fi
